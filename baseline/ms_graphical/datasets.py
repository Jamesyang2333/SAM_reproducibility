...
def LoadDmv(filename_or_df='../../sam_single/datasets/Vehicle__Snowmobile__and_Boat_Registrations.csv'):
...
def LoadCensus(filename_or_df='census.csv'):
    if isinstance(filename_or_df, str):
        filename_or_df = '../../sam_single/datasets/{}'.format(filename_or_df)
    else:
        assert (isinstance(filename_or_df, pd.DataFrame))
